...
const TASKS_TABLE_NAME = `${process.env.MOBILE_HUB_DYNAMIC_PREFIX}-task`;
...
app.get('/items/userTask', (req, res) => {
...
app.post('/items/addTask', (req, res) => {
  if (!req.body.title) {
    res.status(400).json({
  ...
  Object.keys(task).forEach(key => (task[key] === '' && delete task[key]));

  task.userId = req.apiGateway.event.requestContext.identity.cognitoIdentityId || UNAUTH;
  task.taskId = uuid.v1();
